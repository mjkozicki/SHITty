...
### Add Product to Cart
```bash
curl -X POST http://localhost:8080/api/v1/cart/add?user_id=user123 \
...
### Search Products
```bash
curl "http://localhost:8080/api/v1/search?q=iPhone&user_id=user123"
```

### Get Recommendations
```bash
curl http://localhost:8080/api/v1/recommendations/user123
```

### Checkout
```bash
curl -X POST http://localhost:8080/api/v1/checkout?user_id=user123
```
